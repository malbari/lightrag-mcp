...
def parse_args():
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("LIGHTRAG_API_HOST", DEFAULT_HOST),
        help=f"LightRAG API host (default: {DEFAULT_HOST})",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("LIGHTRAG_API_PORT", DEFAULT_PORT)),
        help=f"LightRAG API port (default: {DEFAULT_PORT})",
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("LIGHTRAG_API_KEY", DEFAULT_API_KEY),
...
# MCP HTTP server settings (only from env vars, not CLI)
MCP_HOST = os.environ.get("MCP_HOST", "0.0.0.0")
MCP_PORT = int(os.environ.get("MCP_PORT", "8092"))
